---
title: "FDA_Project"
author: "Dharaneesh A, Vishwanath A L, Shravani Swaroop Urala"
date: "09/09/2021"
output:
...
## the Datasets
```{r}
waterData = read.csv("/Users/shravaniurala/Downloads/water_dataX.csv")
str(waterData)
```
## Cleaning Water data
```{r}
...
```{r}
cropData = read.csv("/Users/shravaniurala/Downloads/crop_production.csv")
str(cropData)
```
## Basic analysis of Area and Production relation
```{r}
summary(lm(Production~Area, cropData))
```
## It can be seen that area has a high relation with the crop prduction, like it usually has

## DIfferent states information present in water dataset
```{r}
...
## Analysing the common years information, which can be used for combining
```{r}
...
## Removing the missing data here, as it is comparatively very low, and can't be replaced with other data, as it would lead it wrong conclusions
```{r}
nrow(cropData)
sum(is.na(cropData))
...
sCrop_data = req_Crop_data %>% group_by(Crop_Year, State_Name) %>%
  summarize(totalArea=mean(Area), totalProd=mean(Production))
sWater_data = sWater_data %>% group_by(year, STATE) %>%
  summarize(Temp=mean(Temp), DO=mean(DO), pH = mean(pH),Conductivity=mean(Conductivity), BOD = mean(BOD), Nitrates=mean(Nitrates), FecalColiform = mean(FecalColiform), TotalColiform = mean(TotalColiform) )
...
# Basic relation ship analysis
```{r}
AreaRel = lm(totalProd~totalArea, com_data)
summary(AreaRel)
```

## Checking for basic relationship between totalProduction and all the other attributes
```{r}
AllRel = lm(totalProd~., com_data[,c(-2)])
summary(AllRel)
```
```{r}
str(com_data)
...
## Function for analysing the linear relationships 
```{r}
cropWiseRel = function(crop){
  DrumStickData = req_Crop_data %>%
  filter(Crop == crop)
  DrumStickData = DrumStickData %>% group_by(Crop_Year, State_Name) %>%
    summarize(totalArea=mean(Area), totalProd=mean(Production))
  DrumStickData <- rename(DrumStickData,STATE=State_Name, year=Crop_Year)
  DrumStickData$STATE <- tolower(DrumStickData$STATE)
  #View(DrumStickData)
  com_data <- merge(sWater_data,DrumStickData,by=c("year","STATE"))
  nrow(com_data)
  DS_AreaRel = lm(totalProd~totalArea, com_data)
  #summary(DS_AreaRel)
  print(paste(crop, " : "))
  print(DS_AreaRel$coefficients)
  print(cor(com_data$totalArea, com_data$totalProd))
  #cropWiseRel = DS_AreaRel
}
```


## This is done to analyse the relation between crops area and their production
```{r}
for(i in crops){
  try(cropWiseRel(i))
...
## the results here are too discrete
